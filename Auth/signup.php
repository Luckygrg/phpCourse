...
if (isset($_POST["submit"])){
    ...
    if($fullname != "" && $email != "" && $password != ""){
        ...
        $select_result = $conn-> query(query: $select_query);
        if($select_result-> num_rows == 0){
            if ($password == $confirmpassword){  //INSERT INTP USERS (COLUMNS NAME) VALUES();
                ...
                header(header: "Location:signup.php?msg=both password shpuld be same. "); 
            }
        }else {
            header(header: "Location:signup.php?msg=empty_taken"); 
        }
